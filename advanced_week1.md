...
```
✅ 개념
- 다른 쿼리 안에 들어가는 쿼리(괄호 안). 결과를 상위 쿼리가 다시 사용

✅ 종류
- 스칼라(값 1개) / 행(row) / 테이블(여러 행·열)
- 비상관(독립 실행) vs 상관(바깥 행마다 재실행)
...
⚠️ 주의
- 과도한 상관 서브퀴리는 성능 저하 -> 가능하면 조인/CTE로 대체 검토 
```
...
✅ 개념
- WITH로 이름 붙인 임시 결과집합
- 아래 쿼리에서 재사용·가독성↑, 재귀 가능

✅ 장점
- 읽기 쉬움(단계 분리)
- 같은 중간결과 여러 번 참조 가능
- 재귀 쿼리 지원(WITH RECURSIVE)
```

```
✅ 서브쿼리 vs CTE
- 한 번만 간단히 쓰면 -> 서브 쿼리
- 여러 번 참조 / 단계 나눔 / 재귀 필요 -> CTE
```
<br>
...
- 장점
  - 가독성↑: 단계별로 쿼리 분해 가능
  - 재사용 가능: 동일 중간결과를 여러 곳에서 참조
  - 재귀 쿼리 지원(`WITH RECURSIVE`)
- 단점
  - 엔진/버전에 따라 **물리화**되어 최적화 덜 될 수 있음
  - 인덱스 직접 생성 불가(임시 결과집합)
  - 단 한 번 쓰는 단순 케이스엔 오히려 과함

✅ 서브쿼리
- 장점
  - 한 번만 필요한 중간결과에 간단·짧은 문법
  - 대부분 상황에서 최적화가 잘 녹아들어 성능 무난
  - 외부 의존 없이 인라인 작성
- 단점
  - 동일 로직을 여러 번 쓰면 중복/가독성↓
  - 재귀 불가
  - 복잡해지면 디버깅 난이도↑
```
